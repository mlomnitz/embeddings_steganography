...
class encoder_decoder():
    def __init__(self, encoder=None, decoder=None,
        ...
        self.Hnet.load_state_dict(torch.load(
            '{}/netH.pth.tar'.format(dis_path)))
        self.Hnet.eval()
        #
        self.Rnet.load_state_dict(torch.load(
            '{}/netR.pth.tar'.format(dis_path)))
        self.Rnet.eval()
        #
    ...
    def init_cover(cover_dir='', cover_transform=None):
        ...
        coverset = GIF_dataset(transform=cover_transform,
                               folder_path=cover_dir)
    ...
    def encode_message(self, message='', n_images=0, save_path='./Example'):
        ...
        for idx, cover in enumerate(self.coverloader):
            ...
            if upper > len(converted):
                break
            cover, secret, container = self.hide_message(
                cover, converted[lower:upper])
            fig = save_frame(
                container[0], output='{}/frame{}.png'.format(temp_dir, idx))
        progress_bar.close()
